...
def ensure_str(text) -> str:
    ...
    If it's int/float/None, convert to string.
    """
...
def normalize_answer(s: str) -> str:
    ...
    s = ensure_str(s)  # Ensure input is string
...
def first_int_match(prediction):
    prediction = ensure_str(prediction)
...
def compute_scores(preds_path, data_name: str, model_name: str) -> float:
...
if __name__ == "__main__":
    ...
    p.add_argument("--output_dir", type=str, default="./results/infinitebench", help="Where to dump the prediction results.")
    p.add_argument(
        "--model_path",
        type=str,
    ...
    p.add_argument("--method", type=str, default="HeteroCache")
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--use_fast_tokenizer", type=bool, default=True)
    args = p.parse_args()
    ...
    for task in tasks:
        result_dir = Path(args.output_dir, args.model_name.lower())
        preds_path = result_dir / f"preds_{task}_{args.method}.jsonl"
        
        if not preds_path.exists():
            print(f"Warning: Predictions not found in: {preds_path}, skipping.")
            continue
        
        try:
            score = compute_scores(preds_path, task, args.model_name.lower())
            current_run_scores[task] = score
        except Exception as e:
            print(f"Error computing score for {task}: {e}, skipping.")
            continue
    ...
    if csv_path.exists():
        try:
            ...
            print(f"Error reading CSV: {e}, creating new one.")
    ...
    for task, score in current_run_scores.items():
        if task not in df.columns:
            df[task] = None
        df.loc[method_name, task] = score

    cols_to_mean = [c for c in df.columns if c != 'Avg']
    
    df_numeric = df[cols_to_mean].apply(pd.to_numeric, errors='coerce')
    
    df['Avg'] = df_numeric.mean(axis=1)

    cols = [c for c in df.columns if c != 'Avg'] + ['Avg']
    df = df[cols]

    df = df.reset_index().rename(columns={"index": "method"})
    if "index" in df.columns and "method" not in df.columns:
        df.rename(columns={"index": "method"}, inplace=True)
        
    df.to_csv(csv_path, index=False)
    print(f"Successfully saved aggregated scores to {csv_path}")